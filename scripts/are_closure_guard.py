#!/usr/bin/env python3
"""Canonical-lane closure guard for the anabelian reconstruction workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['ARE_G1', 'ARE_G2', 'ARE_G3', 'ARE_G4', 'ARE_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['ARE_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['ARE_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['ARE_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['ARE_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['ARE_G1'], 'G2': gates['ARE_G2'], 'G3': gates['ARE_G3'], 'G4': gates['ARE_G4'], 'G5': gates['ARE_G5'], 'G6': 'NA', 'GCoh': gates['ARE_G6'], 'GM': gates['ARE_GM']}
    normalized_blockers = {'G1': blockers['ARE_G1'], 'G2': blockers['ARE_G2'], 'G3': blockers['ARE_G3'], 'G4': blockers['ARE_G4'], 'G5': blockers['ARE_G5'], 'G6': [], 'GCoh': blockers['ARE_G6'], 'GM': blockers['ARE_GM']}
