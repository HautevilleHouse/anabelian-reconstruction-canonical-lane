...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PREPRINT.md` (`ARE_G4`, `ARE_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving persistence of scheme reconstruction from admissible fundamental-group data through a multi-lane anabelian reconstruction super-architecture`.
...
- endpoint-transfer lock: `anabelian_lock`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`ARE_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad countermodels are excluded.
`ARE_G5` closes when `anabelian_lock` transfers the surviving endpoint to the intended target class: rigid limit transfers to the intended endpoint class.
Together they feed the strict margin `M_ARE`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `anabelian_lock`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `ARE_G4` and `ARE_G5` deliver the boundary object needed by the final margin.

## Current Instantiation

- rigidity gate: `ARE_G4`
- rigidity artifact key: `rho_rigidity`
- transfer gate: `ARE_G5`
- transfer artifact key: `anabelian_lock`
