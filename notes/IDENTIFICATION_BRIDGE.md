...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of scheme reconstruction from admissible fundamental-group data through a multi-lane anabelian reconstruction super-architecture`.
...
2. the endpoint-transfer lock `anabelian_lock` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_ARE` remains positive.
...
Payload: verify `anabelian_lock` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_ARE` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `anabelian_lock`
