...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PREPRINT.md` (`ARE_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of scheme reconstruction from admissible fundamental-group data through a multi-lane anabelian reconstruction super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`ARE_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_ARE`.
...
If near-failure extraction and no-Zeno spacing hold, then `ARE_G3` prevents hidden drift before rigidity is applied.
