...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PREPRINT.md` (`ARE_G2`).

## Goal
Expand the compressed capture/restart language into the local-to-global transport gate for `proving persistence of scheme reconstruction from admissible fundamental-group data through a multi-lane anabelian reconstruction super-architecture`.
...
`ARE_G2` closes when `sigma_sections` survives admissible losses and restart corrections: defect stays above capture floor across admissible losses.
This is the transport contribution to `M_ARE`.
...
If transport accounting and restart preservation hold, then `ARE_G2` carries local control forward without breaking admissibility.
