...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PREPRINT.md` (`ARE_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of scheme reconstruction from admissible fundamental-group data through a multi-lane anabelian reconstruction super-architecture`.
...
`ARE_G1` closes when `kappa_reconstruction` satisfies the response-floor requirement: projected response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_ARE`.
...
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_reconstruction`, then `ARE_G1` supplies the projected/protected-core input to `M_ARE`.
